...
const ReviewsSection = () => {
  const getReviews = () => {
    const randomReview = Math.floor(Math.random() * REVIEWS.client.length);
    return (
      <Col md={4}>
        <Card>
          <Card.Body>
            <Card.Text>{REVIEWS.client[randomReview].feedback}</Card.Text>
            <svg
            ...
            <Card.Title>{REVIEWS.client[randomReview].name}</Card.Title>
          </Card.Body>
        </Card>
      </Col>
    );
